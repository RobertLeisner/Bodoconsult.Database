...
{
    /// <summary>
    /// Fake repository for guids
    /// </summary>
    /// <typeparam name="T">POCO entity type</typeparam>
    public class FakeRepositoryGuid<T> : IRepositoryGuid<T> where T : class, IEntityRequirementsGuid, new()
    {
        ...
        public FakeRepositoryGuid(IList<T> context, IUnitOfWork unitOfWork)
        {
            if (context == null)
            {
        ...
        public T Add(T entity)
        {
            ...
            if (_context.Any(x => x.Uid == entity.Uid))
            {
        ...
        private T AddInternal(T entity)
        {
            lock (_sharedLock)
            {
                if (string.IsNullOrEmpty(entity.Uid.ToString()))
                {
                    entity.Uid = new Guid();
                }
                _context.Add(entity);
        ...
        public T AddOrUpdate(T entity)
        {
            ...
            var isEntity = _context.Any(x => x.Uid == entity.Uid);
        ...
        {
            var entry = _context.SingleOrDefault(s => s.Uid == entity.Uid);
        ...
        {
            var method = typeof(IEntityRequirementsGuid).IsAssignableFrom(type) ? 
                typeof(FakeUnitOfWork).GetMethod("GetRepositoryGuid") : 
                typeof(FakeUnitOfWork).GetMethod("GetRepository");
