...
namespace Bodoconsult.Database.Ef.Infrastructure
{
    ...
    /// <summary>
    /// Current impl of <see cref="IDbContextScope&lt;T&gt;"/>. Transactions are supported only for certain execution strategies (i.e. NOT for retrying execution strategie)
    /// </summary>
    public class DbContextScope<T> : IDbContextScope<T> where T : DbContext
    {
        ...
        private readonly DbContextScope<T> _parentScope;

        private readonly DbContextCollection<T> _dbContexts;
        ...
        /// </summary>
        public IContextConfig ContextConfig => _dbContextFactory.AppGlobals.ContextConfig;


        public ConcurrencyConflictSolvingType ConcurrencyConflictSolvingType { get; }


        public IDbContextCollection<T> DbContexts => _dbContexts;

        public DbContextScope(IDbContextWithConfigFactory<T> dbContextFactory) :
        ...
        public DbContextScope(IDbContextWithConfigFactory<T> dbContextFactory, ConcurrencyConflictSolvingType concurrencyConflictSolvingType) :
            this(joiningOption: DbContextScopeOption.JoinExisting, readOnly: false, isolationLevel: null, dbContextFactory: dbContextFactory, true, concurrencyConflictSolvingType)
        { }

        public DbContextScope(bool readOnly, IDbContextWithConfigFactory<T> dbContextFactory)
        ...
        public DbContextScope(bool readOnly, IDbContextWithConfigFactory<T> dbContextFactory, ConcurrencyConflictSolvingType concurrencyConflictSolvingType)
            : this(joiningOption: DbContextScopeOption.JoinExisting, readOnly: readOnly, isolationLevel: null, dbContextFactory: dbContextFactory, true, concurrencyConflictSolvingType)
        { }


        public DbContextScope(DbContextScopeOption joiningOption, bool readOnly, IsolationLevel? isolationLevel,
            IDbContextWithConfigFactory<T> dbContextFactory) :
            this(joiningOption, readOnly: readOnly, isolationLevel, dbContextFactory: dbContextFactory, true, ConcurrencyConflictSolvingType.DatabaseWins)
        //this(joiningOption: DbContextScopeOption.JoinExisting, readOnly: readOnly, isolationLevel: null, dbContextFactory: dbContextFactory, true, ConcurrencyConflictSolvingType.DatabaseWins)
        {

        }


        public DbContextScope(DbContextScopeOption joiningOption, bool readOnly, IsolationLevel? isolationLevel, IDbContextWithConfigFactory<T> dbContextFactory,
            bool autoDetectChanges, ConcurrencyConflictSolvingType concurrencyConflictSolvingType)
        {
            ...
            ConcurrencyConflictSolvingType = ConcurrencyConflictSolvingType.DatabaseWins;
            ...
            {
                _nested = false;
                _dbContexts = new DbContextCollection<T>(readOnly, isolationLevel, dbContextFactory, autoDetectChanges);
            }
        ...
        public int SaveChanges()
        {
            ...
            if (!_nested)
            {
                c = CommitInternal(ConcurrencyConflictSolvingType.DatabaseWins);
            }
        ...
        public int SaveChanges(ConcurrencyConflictSolvingType concurrencyConflictSolvingType)
        {
            ...
            if (!_nested)
            {
                c = CommitInternal(concurrencyConflictSolvingType);
            }
        ...
        {
            return SaveChangesAsync(CancellationToken.None, ConcurrencyConflictSolvingType);
        }
            ...
            ConcurrencyConflictSolvingType concurrencyConflictSolvingType)
        {
            ...
            if (!_nested)
            {
                c = await CommitInternalAsync(concurrencyConflictSolvingType, cancelToken).ConfigureAwait(false);
            }
        ...
        private int CommitInternal(ConcurrencyConflictSolvingType concurrencyConflictSolvingType)
        {
            return _dbContexts.Commit(concurrencyConflictSolvingType);
        }

        private Task<int> CommitInternalAsync(CancellationToken cancelToken)
        {
            return _dbContexts.CommitAsync(ConcurrencyConflictSolvingType.DatabaseWins, cancelToken);
        }


        private Task<int> CommitInternalAsync(ConcurrencyConflictSolvingType concurrencyConflictSolvingType, CancellationToken cancelToken)
        {
            return _dbContexts.CommitAsync(concurrencyConflictSolvingType, cancelToken);
        }
            ...
            foreach (var contextInCurrentScope in _dbContexts.InitializedDbContexts.Values)
            {
                ...
                foreach (var toRefresh in entities)
                {
                    ...
                    if (stateInCurrentScope != null)
                    {
                        ...
                        if (entityEntry is { State: EntityState.Unchanged })
                        {
...
" + Environment.StackTrace;
        ...
        private static readonly ConditionalWeakTable<InstanceIdentifier, DbContextScope<T>> DbContextScopeInstances = new();
        ...
        /// </summary>
        internal static void SetAmbientScope(DbContextScope<T> newAmbientScope)
        {
        ...
        {
            var current = (InstanceIdentifier)CallContext.GetData(AmbientDbContextScopeKey);
        ...
        /// </summary>
        internal static DbContextScope<T> GetAmbientScope()
        {
            // Retrieve the identifier of the ambient scope (if any)
            var instanceIdentifier = (InstanceIdentifier)CallContext.GetData(AmbientDbContextScopeKey);
